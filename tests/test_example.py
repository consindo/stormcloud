...
from stormcloud import AboutStormcloudDialog

class TestExample(unittest.TestCase):
    def setUp(self):
        self.AboutStormcloudDialog_members = [
        'AboutDialog', 'AboutStormcloudDialog', 'gettext', 'logger', 'logging']

    def test_AboutStormcloudDialog_members(self):
        all_members = dir(AboutStormcloudDialog)
        public_members = [x for x in all_members if not x.startswith('_')]
        public_members.sort()
        self.assertEqual(self.AboutStormcloudDialog_members, public_members)
